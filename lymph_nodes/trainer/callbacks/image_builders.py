...
from lymph_nodes.trainer.callbacks.vis_mode import VisualisationMode
...
class JpegImageBuilder(ImageBuilder):
    def __init__(
        self, vis_mode: VisualisationMode, save_dir: Path | str, metadata: dict
    ) -> None:
        super().__init__(vis_mode, save_dir, "")

    def update(self, data: torch.Tensor, metadata: dict) -> None:
        data = self._preprocess_data(data)
        for img, filename in zip(data, metadata["filename"], strict=False):
            vips_im = pyvips.Image.new_from_array(img)
            vips_im = self._finalize_image(vips_im)
            vips_im.jpegsave(Path(self.save_dir) / filename)
