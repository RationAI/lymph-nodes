...
def _map_lymph_node_type(value: object) -> int:
    value_str = str(value).lower().strip()

    if value_str in ["negative"]:
        return 0
    if value_str in ["positive"]:
        return 1

    raise ValueError(f"Unknown lymph node type label: {value!r}")
...
def process_slides(slides: pd.DataFrame, mode: LabelMode | None = None) -> pd.DataFrame:
    ...
    match mode:
        case LabelMode.TYPE:
            slides = slides.copy()
            slides["type_label"] = slides["type"].map(_map_lymph_node_type)
